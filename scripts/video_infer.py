...
# ----------------- 主推理流程 -----------------
def infer_video(engine_path, video_path, save_path):
    ...
    for _ in range(5):
        cuda_preprocess(dummy_img, d_input, ow=640, oh=384)
    ...
    while True:
        ...
        start_event.record()
        cuda_preprocess(img, d_input, ow=640, oh=384)
        context.execute_async_v2(bindings=bindings, stream_handle=stream.handle)
        cuda.memcpy_dtoh_async(output_data, d_output, stream)
        stream.synchronize()
        end_event.record()
        ...
        ms = start_event.time_till(end_event)
        ...
        for det in detections:
            ...
            cv2.putText(img, caption, (int(left), int(top)-5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)

        writer.write(img)
        print(f"Frame {frame_id}: total {ms:.2f} ms")
        frame_id += 1
    ...
    print(f"\nInference complete! {frame_id} frames in {total_time:.2f}s, Average FPS: {avg_fps:.2f}")
...
if __name__ == "__main__":
    ...
    video_path  = '/home/zhihong/yolov11n/dataset/camera_videos/88.mp4'
    save_path   = '/home/zhihong/yolov11n/outputs/train1/infer_videos/88_infered.mp4'
    infer_video(engine_path, video_path, save_path)
